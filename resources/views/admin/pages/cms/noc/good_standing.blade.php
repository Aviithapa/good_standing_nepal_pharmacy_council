        ...
        <div class="container--fluid"
            ...
            <div class="noc_content">
                ...
                <div class="ref_container"
                    style="width: 100%; color: #0000ff; font-weight: 600; margin-bottom: 1rem;  font-style: italic;">
              ...
              background-blend-mode: multiply;">
                ...
                <div class="noc_body" style="font-size: 14px; line-height: 30px;">
                    <p style="margin-bottom: 1rem; text-align:justify;">
                        ...
                        <span
                            style="font-weight:700">{{ $nocData->first_name . ' ' . $nocData->middle_name . ' ' . $nocData->last_name }}
                        </span>
                        a Nepalese national holding
                        Passport no <span style="font-weight:700">{{ $nocData->national_id }}</span> is registered with
                        registration number <span style="font-weight:700">{{ $nocData->registration_number }}</span> as
                        a {{ $nocData->position }} in Nepal Pharmacy Council in accordance with
                        Nepal Pharmacy Council act, 2057 B.S.(2000 A.D).
                        <br /> <br />
                        {{ $nocData->title === 'mr' ? 'His' : 'Her' }} date of birth according to our record is
                        {{ $dob }}.
                        {{ $nocData->title === 'mr' ? 'He' : 'She' }} has passed <span
                            style="font-weight:700">{{ $nocData->level }}</span> from {{ $nocData->university }}
                        in {{ $nocData->passed_year }}. {{ $nocData->title === 'mr' ? 'He' : 'She' }} is registered as
                        a {{ $nocData->position }}. There is nothing against
                        {{ $nocData->title === 'mr' ? 'him' : 'her' }} in Nepal Pharmacy Council till present day.
                        <br /> <br /> I wish success in {{ $nocData->title === 'mr' ? 'his' : 'her' }} future career.
