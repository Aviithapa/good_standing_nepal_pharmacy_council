        ...
        <form action="{{ route('good-standing-main.index') }}" method="GET" novalidate>
            <div class="row" style="padding: 20px 10px 0px 10px;">
                <div class="col-lg-4 col-md-4 col-sm-6">
                    <div class="mb-3">
                        <input type="text" class="form-control" id="validationCustom01" placeholder="Name" name="keyword"
                ...
                <div class="col-lg-4 col-md-4 col-sm-6">
                    <div class="mb-3">
                        <select class="form-control" id="statusSelect" name="status">
                            <option value="" {{ request()->get('status') === null ? 'selected' : '' }}>Select Status
                            </option>
                ...
                <div class="col-lg-3 col-md-3 col-sm-6">
                    <div class="mb-3">
                        <input type="text" class="form-control" id="validationCustom01" placeholder="Citizenship"
                            name="citizenship" value="{{ request()->get('citizenship') }}">
                    </div>
                </div>
                ...
                <div class="p-3">
                    <div class="card-widgets">
                        <form action="{{ url('export-data-good') }}" method="POST">
                            ...
                            <button type="submit" class="btn btn-primary" style="color: white;">Export Data Good
                                Standing</button>
                        </form>
                                ...
                                @foreach ($goodStanding as $index => $data)
                                    ...
                                    <tr class="{{ $rowClass }}">
                                            ...
                                            @if ($data->status === 'approved')
                                                <a target="_blank"
                                                    ...
                                                    <span class="badge bg-purple-subtle text-purple"
                                                        style="font-size: 18px; text-transform: capitalize;">View</span>
                                                </a>
                                                <a target="_blank" href="{{ getImage($data->pdf_link) }}">
                                                    <span class="badge bg-pink-subtle text-pink"
                                                        style="font-size: 18px; text-transform: capitalize;">Print</span>
                                                </a>
                                                @if (Auth::user()->mainRole()->name === 'super_admin')
                                                    <a class="btn-approve" data-id="{{ $data->id }}"
                                                        data-status="approved" style="cursor: pointer;">
                                                        <span class="badge bg-pink-subtle text-pink"
                                                            style="font-size: 18px; text-transform: capitalize;">Re
                                                            Generate</span>
                                                    </a>
                                                @endif
                                                <a class="download"
                                            ...
                                            @else
                                                ...
                                                @if (Auth::user()->mainRole()->name === 'super_admin')
                                                    <a class="btn-approve" data-id="{{ $data->id }}"
                                                        data-status="approved" style="cursor: pointer;">
                                                        <span class="badge bg-pink-subtle text-pink"
                                                            style="font-size: 18px; text-transform: capitalize;">Approve</span>
                                                    </a>
                                                @endif
                                                <a class="btn-reject" data-id="{{ $data->id }}" data-status="rejected"
                                                    style="cursor: pointer;">
                                                    <span class="badge bg-pink-subtle text-pink"
                                                        style="font-size: 18px; text-transform: capitalize;">Reject</span>
                                                </a>
                                            @endif
                                        </td>
                                        ...
                                        <td><span class="badge bg-success-subtle text-success"
                                                style="font-size: 18px; text-transform: capitalize;">{{ $data->good_standing ? 'Good Standing' : 'NOC' }}</span>
                                        </td>
    ...
    <script>
        $(document).ready(function() {
            $('.btn-approve').on('click', function() {
                ...
                const url = '{{ route('noc.approve', ['id' => ':id', 'status' => 'approved']) }}'.replace(
                    ':id', id);
            ...
            $('.btn-reject').on('click', function() {
                const id = $(this).data('id');
                const url = '{{ route('noc-main.update', ['noc_main' => ':id', 'status' => 'rejected']) }}'
                    .replace(':id', id);
